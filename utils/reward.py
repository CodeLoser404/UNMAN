...
def calculate_angle_reward(my_pos, enemy_pos, my_attitude):
    ...
    angle_reward = 10 * cos_theta if np.abs(cos_theta) > 0.95 else cos_theta
    return angle_reward
...
def calculate_reward(prev_my_state, prev_enemy_state, my_state, enemy_state):
    ...
    angle_reward = 10 * calculate_angle_reward(my_pos, enemy_pos, my_attitude)
    ...
    if prev_enemy_hp - enemy_hp == 0:
        ...
        if not is_hit:  # 第一次击中
            ...
            hit_step_count = 0
            miss_step_count = 0
        hp_reward += 100 + hit_step_count * 10  # 连续命中奖励
        hit_step_count += 1

    if enemy_hp == 0: # 击落敌机
        hp_reward += 1000

    reward = distance_reward + angle_reward + hp_reward
